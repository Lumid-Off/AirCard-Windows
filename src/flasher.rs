    ...
    L: FnMut(&str),
{
    ...
    let afc = AfcClient::new(&mut session).context("Failed to open AFC connection")?;

    let snapshot = snapshot_books(&afc).context("Failed to snapshot Books state before staging")?;
    let recovery = crate::airlift::save_recovery(&snapshot, udid)?;
    log(&format!("Books recovery backup: {}", recovery.display()));
    ...
    let write_res = (|| -> Result<()> {
        ...
        stage_streaming_zip(&mut session, &source, &archive_data)
    ...
    if let Err(restore) = restore_res {
        bail!(
            "Books state restoration failed: {restore:#}; operation result: {:?}",
            write_res.err()
        );
    }
    write_res?;
    log(&format!("Submitted artwork asset: {}", leaf_name));
    ...
    L: FnMut(&str),
{
    ...
    let write_res = (|| -> Result<()> {
        ...
        let assets_refs: Vec<(&str, &str)> = assets_to_sync
            ...
            .collect();
        sync_assets_via_airtraffic(session.udid(), session.transport(), &assets_refs, &mut log)
    ...
    write_res?;
    log(&format!(
        "Submitted batch of {} file(s); destination appearance requires verification.",
        items.len()
    ));
